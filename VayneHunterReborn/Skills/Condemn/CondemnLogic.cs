...
namespace VayneHunter_Reborn.Skills.Condemn
{
    class CondemnLogic
    {
        ...
        public static Obj_AI_Hero GetCondemnTarget2()
        {
                ...
                if (Prediction.Hitchance >= HitChance.VeryHigh)
                {
                    if (endPosition.IsWall())
                    {
                        var condemnRectangle =
                            new VHRPolygon(
                                VHRPolygon.Rectangle(
                                    target.ServerPosition.To2D(), endPosition.To2D(), target.BoundingRadius));
                        ...
                        for (float i = 0; i < pushDistance; i += step)
                        {
                            ...
                            if (endPositionEx.IsWall())
                            {
                                ...
                                if (
                                    condemnRectangle.Points.Count(
                                        point =>
                                            NavMesh.GetCollisionFlags(point.X, point.Y).HasFlag(CollisionFlags.Wall)) >=
                                    condemnRectangle.Points.Count() * (20 / 100f))
                                {
                                    return target;
