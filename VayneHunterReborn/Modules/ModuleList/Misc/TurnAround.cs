...
namespace VayneHunter_Reborn.Modules.ModuleList.Misc
{
    class TurnAround : IModule
    {
        ...
        private void OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            {
                if (!sender.IsValidTarget() || !(sender is Obj_AI_Hero) || !ObjectManager.Player.IsValidTarget(float.MaxValue, false))
                {
                    return;
                }
                ...
                if (currentSpell != null)
                {
                    ...
                    || ObjectManager.Player.Distance(sender.ServerPosition) + 65f <= currentSpell.Range)
                    {
                        ...
                        LeagueSharp.Common.Utility.DelayAction.Add((int)((currentSpell.CastDelay + 0.20f) * 1000), () => ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, LastMovementCommandIssued));
                    }
                }
