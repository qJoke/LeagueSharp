    ...
    class RangesBase : ModuleBase
    {
        ...
        void Drawing_OnDraw(EventArgs args)
        {
            try
            {
                if (ShouldRun())
                {
                    if (MenuExtensions.GetItemValue<bool>("dz191.dza.ranges.enemy"))
                    {
                        DrawEnemyZone();
                    }
        ...
        {
            var staticRange = 360f;
            var polygonsList = Variables.AlliesClose.Select(enemy => new Geometry.Circle(enemy.ServerPosition.To2D(), (dynamic ? (enemy.IsMelee ? enemy.AttackRange * 1.5f : enemy.AttackRange) : staticRange) + enemy.BoundingRadius + 20).ToPolygon()).ToList();
            var pathList = Geometry.ClipPolygons(polygonsList);
            var pointList = pathList.SelectMany(path => path, (path, point) => new Vector2(point.X, point.Y)).Where(currentPoint => !currentPoint.IsWall()).ToList();
            return pointList;
        }
