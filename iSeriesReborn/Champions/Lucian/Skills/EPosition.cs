...
namespace SoloVayne.Skills.Tumble
{
    class EPositionProvider
    {
        ...
        public Vector3 GetEPosition()
        {
            ...
            var enemiesNear =
                HeroManager.Enemies.Where(m => m.IsValidTarget(Orbwalking.GetRealAutoAttackRange(m) + 450f + 65f)).ToList();
            #endregion
            ...
            if (ObjectManager.Player.CountEnemiesInRange(1500f) <= 1)
            {
                ...
                var backwardsPosition = (ObjectManager.Player.ServerPosition.To2D() + 450f * ObjectManager.Player.Direction.To2D()).To3D();
                            ...
                            && t.Distance(ObjectManager.Player) < Orbwalking.GetRealAutoAttackRange(t) + 80f))
                {
                    var QPosition =
                        ObjectManager.Player.ServerPosition.Extend(
                            highHealthEnemiesNear.OrderBy(t => t.Health).First().ServerPosition, 450f);
            ...
            var closeNonMeleeEnemy = DashHelper.GetClosestEnemy(ObjectManager.Player.ServerPosition.Extend(Game.CursorPos, 450f));
            ...
            {
                return ObjectManager.Player.ServerPosition.Extend(Game.CursorPos, 450f).IsSafeEx()
